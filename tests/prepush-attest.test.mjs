...
describe('base-guard fetches lazily and only to disprove a violation (#6764)', () => {
  ...
  test('an explicit upstream sync excludes only the reviewed imported history', () => {
    const fixture = makeCloneWithOrigin({ aheadCommits: 25 });
    const { clone, root } = fixture;
    const upstream = join(root, 'upstream.git');
    git(clone, ['clone', '--quiet', '--bare', clone, upstream]);
    git(clone, ['remote', 'add', 'upstream', upstream]);
    const imported = git(clone, ['rev-parse', 'HEAD']).trim();
    git(clone, ['push', '--quiet', 'upstream', 'HEAD:main']);
    git(clone, ['commit', '--quiet', '--allow-empty', '-m', 'fork integration fix']);
    assert.equal(baseGuard(fixture, ['main', '20']).status, 3, 'sync is opt-in');
    const result = baseGuard(fixture, ['main', '20'], { WM_UPSTREAM_SYNC_SHA: imported });
    assert.equal(result.status, 0);
    assert.equal(result.count, 1);
    // Even a valid sync must not hide unrelated fork work.
    for (let i = 0; i < 20; i += 1) git(clone, ['commit', '--quiet', '--allow-empty', '-m', `extra ${i}`]);
    const tooMuch = baseGuard(fixture, ['main', '20'], { WM_UPSTREAM_SYNC_SHA: imported });
    assert.equal(tooMuch.status, 3);
    assert.equal(tooMuch.count, 21);
    const unreviewed = git(clone, ['rev-parse', 'HEAD']).trim();
    assert.equal(baseGuard(fixture, ['main', '20'], { WM_UPSTREAM_SYNC_SHA: unreviewed }).status, 1,
      'a fork-only SHA cannot suppress the guard');
  });

  test('upstream sync refuses a missing remote or an upstream SHA not integrated locally', () => {
    const fixture = makeCloneWithOrigin({ aheadCommits: 1 });
    const { clone } = fixture;
    const imported = git(clone, ['rev-parse', 'HEAD']).trim();
    assert.equal(baseGuard(fixture, ['main', '20'], { WM_UPSTREAM_SYNC_SHA: imported }).status, 1);
    git(clone, ['remote', 'add', 'upstream', fixture.clone]);
    git(clone, ['branch', '-f', 'main', imported]);
    git(clone, ['checkout', '--quiet', '--detach', 'origin/main']);
    assert.equal(baseGuard(fixture, ['main', '20'], { WM_UPSTREAM_SYNC_SHA: imported }).status, 1);
    assert.equal(baseGuard(fixture, ['main', '20'], { WM_UPSTREAM_SYNC_SHA: 'HEAD' }).status, 1);
  });
