  ...
  base-guard)
    ...
    fetch_base() {
      ...
      if command -v node >/dev/null 2>&1; then
        WM_PREPUSH_FETCH_BASE="$1" WM_PREPUSH_FETCH_REMOTE="${2:-origin}" node - <<'NODE'
...
const result = spawnSync(
  'git',
  ['fetch', '--no-tags', process.env.WM_PREPUSH_FETCH_REMOTE, process.env.WM_PREPUSH_FETCH_BASE, '--quiet'],
  { stdio: 'ignore', timeout, killSignal: 'SIGTERM' },
);
process.exit(result.error ? 1 : (result.status ?? 1));
NODE
      elif command -v timeout >/dev/null 2>&1; then
        timeout 120 git fetch --no-tags "${2:-origin}" "$1" --quiet 2>/dev/null
      elif command -v gtimeout >/dev/null 2>&1; then
        gtimeout 120 git fetch --no-tags "${2:-origin}" "$1" --quiet 2>/dev/null
      else
    ...
    # A deliberate fork sync can import many upstream commits. Exclude only
    # the explicitly reviewed SHA, after proving it belongs to live upstream
    # main AND is already integrated here. Keep counting all fork-only work;
    # this does not change the full origin/main diff used by any test gate.
    if [ -n "${WM_UPSTREAM_SYNC_SHA:-}" ]; then
      if ! [[ "$WM_UPSTREAM_SYNC_SHA" =~ ^[0-9a-f]{40}$ ]] ||
         ! fetch_base main upstream ||
         ! git merge-base --is-ancestor "$WM_UPSTREAM_SYNC_SHA" refs/remotes/upstream/main ||
         ! git merge-base --is-ancestor "$WM_UPSTREAM_SYNC_SHA" HEAD ||
         ! git merge-base --is-ancestor "origin/$base" HEAD; then
        echo "base-guard: upstream sync requires a full SHA integrated from live upstream/main and the current fork base" >&2
        exit 1
      fi
      count=$(git rev-list --count "origin/$base..HEAD" --not "$WM_UPSTREAM_SYNC_SHA") || exit 1
      echo "base-guard: audited upstream sync $WM_UPSTREAM_SYNC_SHA; $count fork-only commits remain" >&2
    fi
